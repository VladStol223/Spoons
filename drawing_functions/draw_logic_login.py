...
def draw_login(screen, login_mode, username_text, password_text, input_active, background_color):
    ...
    btn_y = pass_y + field_h + int(screen_h * 0.04)
...
def logic_login(event, login_mode, username_text, password_text, input_active):
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        ...
        if not login_mode and _main_choice_rects:
            if _main_choice_rects.get("register") and _main_choice_rects["register"].collidepoint(event.pos): return "register", username_text, password_text, None, page
            if _main_choice_rects.get("login") and _main_choice_rects["login"].collidepoint(event.pos): return "login", username_text, password_text, None, page
            if _main_choice_rects.get("offline") and _main_choice_rects["offline"].collidepoint(event.pos): return login_mode, username_text, password_text, None, "input_spoons"

        # In the form
        if login_mode and _form_rects:
            if _form_rects.get("user") and _form_rects["user"].collidepoint(event.pos): return login_mode, username_text, password_text, "username", page
            if _form_rects.get("pass") and _form_rects["pass"].collidepoint(event.pos): return login_mode, username_text, password_text, "password", page
            if _form_rects.get("back") and _form_rects["back"].collidepoint(event.pos): return None, "", "", None, page
            if _form_rects.get("confirm") and _form_rects["confirm"].collidepoint(event.pos): return login_mode, username_text, password_text, None, page  # no-op for now

        # Clicked elsewhere
        if login_mode: return login_mode, username_text, password_text, None, page

    elif event.type == pygame.KEYDOWN and login_mode:
        # Typing into active field
        target = None
        if input_active == "username": target = "username"
        elif input_active == "password": target = "password"
        ...
        if target:
            if event.key == pygame.K_TAB:
                ...
                return login_mode, username_text, password_text, None, page  # Confirm is a no-op for now
